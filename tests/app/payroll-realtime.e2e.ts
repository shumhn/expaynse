// Canonical realtime payroll end-to-end runner
//
// Purpose:
// Prove that payroll accrues in real time for an onboarded employee, with the
// employer controlling the stream lifecycle and the employee owning the private
// recipient account.
//
// What this covers:
// 1. Employer creates an employee and paused stream through app routes
// 2. Employer builds and signs onboarding transactions
// 3. Server-side auto-init tries to initialize the employee private recipient
//    using the system sponsor wallet or company treasury, and falls back to
//    employee manual init only if needed
// 4. Employer resumes the stream and schedules checkpoint crank
// 5. The test polls exact PER state and proves accrual advances autonomously
// 6. Employer pauses the stream and cancels the checkpoint crank
// 7. The test proves accrual stops changing after cancellation
//
// Required env:
// - MONGODB_URI=...
//
...
//
// This is the primary realtime payroll proof for Expaynse.
...
async function main() {
  ...
  const companyResponse = await companyCreatePost(
    await makeAuthenticatedJsonRequest({
      ...
      body: {
        employerWallet,
        name: "Expaynse Realtime E2E Company",
      },
    }),
  );
  assert.strictEqual(
  ...
  await fundPrivateUsdcIfNeeded({
    connection,
    payer: employer,
    ownerWallet: treasurySigner.publicKey.toBase58(),
  ...
  const streamAfterRecipientInit = await getStreamById(employerWallet, streamId);
  assert(
    streamAfterRecipientInit?.recipientPrivateInitializedAt,
