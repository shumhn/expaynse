// Checkpoint crank regression runner
//
// Purpose:
...
// - TEST_OBSERVE_WINDOW_MS=60000
// - TEST_OBSERVE_SAMPLE_MS=5000
...
async function buildCheckpointCrankAndFinalize(args: {
  ...
  if (buildResponse.status !== 201) {
    const buildErrorText = await buildResponse.clone().text();
    console.error(`${args.mode} checkpoint crank build failed:`, {
  ...
  if (finalizeResponse.status !== 200) {
    const finalizeErrorText = await finalizeResponse.clone().text();
    console.error(`${args.mode} checkpoint crank finalize failed:`, {
      status: finalizeResponse.status,
      body: finalizeErrorText,
    });
  }
...
async function buildControlAndFinalize(args: {
  ...
  let commitSignature = "skip";
  if (buildJson.transactions.commitEmployee) {
    commitSignature = await sendBuiltTransaction({
...
async function observeAutonomousCheckpointWindow(args: {
  employerWallet: string;
  streamId: string;
  teeAuthToken: string;
  baseline: ExactStateSnapshot;
  ...
  const samples: ExactStateSnapshot[] = [args.baseline];
  const iterations = Math.max(1, Math.ceil(args.windowMs / args.sampleMs));

  for (let iteration = 1; iteration <= iterations; iteration += 1) {
    await sleep(args.sampleMs);

    const latest = await fetchExactState({
      employerWallet: args.employerWallet,
      streamId: args.streamId,
      teeAuthToken: args.teeAuthToken,
    });
    ...
    console.log(
      `[observe:autonomous-window] ${iteration}/${iterations}`,
      formatSnapshot("latest", latest),
    );
  }
  ...
  assert(
    didCheckpointAdvance(first, last),
    `Expected autonomous checkpoint to advance during ${args.windowMs}ms observation window`,
  );
...
async function main() {
  ...
  const observeWindowMs = envInteger("TEST_OBSERVE_WINDOW_MS", 0);
  const observeSampleMs = envInteger("TEST_OBSERVE_SAMPLE_MS", 5000);

  const employeeName = `Autonomous Checkpoint ${randomUUID().slice(0, 8)}`;
  const connection = new Connection(DEVNET_RPC, "confirmed");

  console.log("=== Checkpoint Crank Regression E2E ===");
  ...
  assert(
    ...
    "Employer must hold devnet USDC on base for checkpoint regression setup",
  );
  ...
  logSection("Create company");

  const companyResponse = await companyPost(
    makeJsonRequest("http://localhost/api/company/create", {
      employerWallet,
      name: `Checkpoint Crank ${randomUUID().slice(0, 8)}`,
    }),
  );
  ...
  assert(
    companyJson.company,
    companyJson.error || "Company create missing payload",
  );

  console.log("Company ready:", companyJson.company);
  ...
  if (!(onboardResponse.status === 200 || onboardResponse.status === 201)) {
    const onboardErrorText = await onboardResponse.clone().text();
    console.error("Onboard build failed:", {
      status: onboardResponse.status,
      body: onboardErrorText,
    });
  }
  ...
  if (onboardFinalizeResponse.status !== 200) {
    const onboardFinalizeErrorText = await onboardFinalizeResponse
      .clone()
      .text();
    console.error("Onboard finalize failed:", {
      status: onboardFinalizeResponse.status,
      body: onboardFinalizeErrorText,
    });
  }
  ...
  if (employeePrivateInitBuildResponse.status !== 201) {
    const employeePrivateInitBuildErrorText =
      await employeePrivateInitBuildResponse.clone().text();
    console.error("Employee private init build failed:", {
      status: employeePrivateInitBuildResponse.status,
      body: employeePrivateInitBuildErrorText,
    });
  }
    ...
    await employeePrivateInitFinalizePatch(
      await makeAuthenticatedJsonRequest({
        ...
        body: {
          ...
          txSignature: employeePrivateInitSignature,
          teeAuthToken: employeeTeeAuthToken,
        },
      }),
    );

  if (employeePrivateInitFinalizeResponse.status !== 200) {
    const employeePrivateInitFinalizeErrorText =
      await employeePrivateInitFinalizeResponse.clone().text();
    console.error("Employee private init finalize failed:", {
      status: employeePrivateInitFinalizeResponse.status,
      body: employeePrivateInitFinalizeErrorText,
    });
  }
  ...
  let observedWindow:
    | {
        first: ExactStateSnapshot;
        last: ExactStateSnapshot;
        samples: ExactStateSnapshot[];
      }
    | undefined;

  if (observeWindowMs > 0) {
    logSection(`Observe autonomous checkpoint window (${observeWindowMs}ms)`);

    observedWindow = await observeAutonomousCheckpointWindow({
      employerWallet,
      streamId,
      teeAuthToken: employerTeeAuthToken,
      baseline: autonomousState,
      windowMs: observeWindowMs,
      sampleMs: observeSampleMs,
    });

    console.log("Autonomous observation window result:", {
      first: formatSnapshot("window-start", observedWindow.first),
      last: formatSnapshot("window-end", observedWindow.last),
      deltaLastAccrualTimestamp: (
        observedWindow.last.lastAccrualTimestamp -
        observedWindow.first.lastAccrualTimestamp
      ).toString(),
      deltaAccruedUnpaidMicro: (
        observedWindow.last.accruedUnpaidMicro -
        observedWindow.first.accruedUnpaidMicro
      ).toString(),
      sampleCount: observedWindow.samples.length,
      sampleMs: observeSampleMs,
    });
  }
  ...
  console.log({
    ...
    observedWindowLastAccrualTimestamp:
      observedWindow?.last.lastAccrualTimestamp.toString(),
    observedWindowAccruedUnpaidMicro:
      observedWindow?.last.accruedUnpaidMicro.toString(),
  ...
  console.log("\nCheckpoint crank regression passed.");
}

main().catch((error: unknown) => {
  console.error(
    "Checkpoint crank regression failed:",
