...
async function main() {
  ...
  const baseRpc = process.env.NEXT_PUBLIC_SOLANA_RPC_URL || process.env.BASE_RPC_URL || "https://api.devnet.solana.com";
  const erRpc = process.env.ER_RPC_URL!;
  
  console.log(`Using Base RPC: ${baseRpc}`);

  if (!process.env.EMPLOYER_KEYPAIR || !process.env.EMPLOYEE_KEYPAIR) {
    throw new Error(
      "Missing EMPLOYER_KEYPAIR or EMPLOYEE_KEYPAIR. Configure them in .env or use the local key defaults."
    );
  }
